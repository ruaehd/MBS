   ...
   <script>
   	$(function(){
   		...
   		$('#btn_img').click(function(){
   			$('.oldimgs_').attr('src','resources/imgs/no-image.png');
   			$('#imgform').modal('show');
   		});
   		$('.btn-edit').click(function(){
   			for(var l=0;l<5;l++){
   				var sr = $('.imgs_'+l).attr('src');
   				$('.oldimgs_'+l).attr('src',sr);
   			}
   			$('#imgform').modal('toggle');
   		});
   		var mlist = new Array();
		<c:forEach items="${mlist}" var="vo">
			var json = new Object();
			json.menu_name="${vo.menu_name}";
			json.menu_price="${vo.menu_price}";
			mlist.push(json);
		</c:forEach>
   		...
   		for(var i=0;i<imgco;i++){
   			$('.oldimgs_'+i).attr("src","getBizImg.do?number=${vo.company_num}&no="+i);
   		}
	...
	function sample4_execDaumPostcode() {
        ...
        $('#next-3').on('click', function(e) {
        	for(var j=0;j<$('input[name="aaa"').length;j++){
        		if($('.mname').eq(j).val() == "" ||$('.price').eq(j).val() == ""){
        			alert("비워진 항목은 '삭제'버튼을 눌러 삭제해주세요");
        			return null;
        		}
        	}
